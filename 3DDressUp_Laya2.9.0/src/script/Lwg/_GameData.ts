import { DataAdmin, DateAdmin, StorageAdmin, TimerAdmin, Tools } from "./Lwg";
import { _3D } from "./_3D";
    ...
    export class _Tweeting {
        ...
        _photo = {
            ...
            take: (Scene: Laya.Scene, index: number) => {
                this._photo.arr[index] && (this._photo.arr[index] as Laya.Texture).destroy();
            ...
            clear: () => {
                for (let index = 0; index < this._photo.arr.length; index++) {
                    const element = this._photo.arr[index] as Laya.Texture;
